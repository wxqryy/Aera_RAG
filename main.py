...
class AIModel:
    def __init__(self):
        ...
        self.rewrite_prompt = [{"role": "system", "content": """Ты — киноассистент, который помогает формулировать запросы для поиска фильмов и анализа кинематографа.
            ...
            """}]
        rewrited_text = self.processor.apply_chat_template(self.rewrite_prompt, tokenize=False, add_generation_prompt=False)
        self.rewrite_tokens = self.processor(
            text=rewrited_text,
            return_tensors="pt"
        ).to("cuda")

        self.answer_prompt = [{"role": "system", "content": """Ты — умный киноассистент, который помогает пользователям находить фильмы и отвечать на вопросы о кино.
...
- **Не используй шаблонные фразы. Пиши живо, естественно и по делу. Не используй разметочных фраз, но сохраняй разметку.**
- **НИКОГДА НЕ УКАЗЫВАЙ РЕЙТИНГ ФИЛЬМА В ОТВЕТЕ!**
        """}]
        answer_text = self.processor.apply_chat_template(self.answer_prompt, tokenize=False,
                                                          add_generation_prompt=False)
        self.answer_tokens = self.processor(
            text=answer_text,
            return_tensors="pt"
        ).to("cuda")
    ...
    def search(self, query, index, top_k=20):
        ...
        for idx, dist in zip(indices[0], distances[0]):
            ...
            adjusted_score += (rating - self.avge_rating) * self.rating_weight
            adjusted_score += (year - self.avge_year) * self.year_weight
    ...
    def get_movie_info(self, movie_id, distance, db_path='database.db', table_name='data'):
        ...
        if row:
            rating = float(row["rating"]) if row["rating"] is not None else self.avge_rating
            year = int(row["year"]) if row["year"] is not None else self.avge_year
    ...
    def predict(self, data):
        ...
        user_input = data
        newMMessage = [{"role": "user", "content": user_input}]
        newText = self.processor.apply_chat_template(
            newMMessage,
            tokenize=False,
            add_generation_prompt=True
        )
        new_tokens = self.processor(
            text=newText,
            return_tensors="pt"
        ).to("cuda")
        ...
        formatted_movies = self.format_movies_for_prompt(
            [movie[1]["id"] for movie in sorted_movies])
